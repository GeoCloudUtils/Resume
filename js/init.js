jQuery(document).ready(function () {
    ...
    waxon_tm_modalbox_about(); function waxon_tm_popupscroll_about() {
        var WW = jQuery(window).width(); var H = jQuery(window).height(); var scrollable = jQuery('.waxon_tm_modalbox_about .scrollable'); var popupBox = jQuery('.waxon_tm_modalbox_about .description_wrap'); if (WW >= 1200) { popupBox.css({ height: H - 140 }); } else { popupBox.css({ height: H }); }
    ...
    waxon_tm_modalbox_news(); function waxon_tm_popupscroll() {
        var WW = jQuery(window).width(); var H = jQuery(window).height(); var scrollable = jQuery('.waxon_tm_modalbox_news .scrollable'); var popupBox = jQuery('.waxon_tm_modalbox_news .description_wrap'); if (WW >= 1200) { popupBox.css({ height: H - 140 }); } else { popupBox.css({ height: H }); }
        scrollable.each(function () {
            var element = jQuery(this); var wH = jQuery(window).height(); element.css({ height: wH - 140 }); if (WW >= 1200) { element.css({ height: wH - 140 }); } else { element.css({ height: wH }); }
            element.niceScroll({ touchbehavior: false, cursorwidth: 0, autohidemode: true, cursorborder: "0px solid #fff" });
        });
    }
    waxon_tm_popupscroll(); function waxon_tm_hero_slider() {
        var section = $('.fn_cs_personal_slider'); section.each(function () {
            var element = $(this); var mainSliderSelector = element.find('.swiper-container'); var transform = 'Y'; var direction = 'horizontal'; var interleaveOffset = 0.5; if (direction === 'horizontal') { transform = 'X'; }
    ...
    waxon_tm_color_switcher(); function waxon_tm_switcher_opener() {
        var settings = jQuery('.waxon_tm_settings'); var button = settings.find('.link'); var direction = settings.find('.direction li a'); var light = settings.find('.direction li a.light'); var dark = settings.find('.direction li a.dark'); button.on('click', function () {
            var element = jQuery(this); if (element.hasClass('opened')) { element.removeClass('opened'); element.closest('.waxon_tm_settings').removeClass('opened'); } else { element.addClass('opened'); element.closest('.waxon_tm_settings').addClass('opened'); }
            return false;
        }); direction.on('click', function () { var element = jQuery(this); if (!element.hasClass('active')) { direction.removeClass('active'); element.addClass('active'); } }); dark.on('click', function () { var el = jQuery(this); jQuery('body').addClass('dark'); jQuery('.waxon_tm_partners').addClass('opened'); el.closest('.waxon_tm_settings').addClass('changed'); return false; }); light.on('click', function () { var ele = jQuery(this); jQuery('body').removeClass('dark'); jQuery('.waxon_tm_partners').removeClass('opened'); ele.closest('.waxon_tm_settings').removeClass('changed'); return false; });
    }
    waxon_tm_switcher_opener(); function waxon_tm_cursor_switcher() {
        var wrapper = jQuery('.waxon_tm_all_wrap'); var button = jQuery('.waxon_tm_settings .cursor li a'); var show = jQuery('.waxon_tm_settings .cursor li a.show'); var hide = jQuery('.waxon_tm_settings .cursor li a.hide'); button.on('click', function () {
    ...
    waxon_tm_cursor_switcher(); function waxon_tm_kenburn_slider() {
        var mySlider = jQuery('.vegas-slide-inner'); if (mySlider.length) {
            var dataImages = jQuery('.vegas-slide-inner').data('images'); var nameArray = dataImages.split(','); var html = []; for (var i = 0; i < nameArray.length; i++) { html.push({ src: nameArray[i] }); }
    ...
    waxon_tm_kenburn_slider(); jQuery('.anchor_nav').onePageNav(); function waxon_tm_filter_opener() {
        var button = jQuery('.waxon_tm_portoflio .portfolio_filter .wrapper a'); var list = jQuery('.waxon_tm_portoflio .portfolio_filter ul li'); button.on('click', function () {
            var element = jQuery(this); if (element.hasClass('opened')) { element.removeClass('opened'); list.removeClass('opened'); } else { element.addClass('opened'); list.each(function (i) { var ele = jQuery(this); setTimeout(function () { ele.addClass('opened'); }, i * 100); }); }
            return false;
        });
    }
    ...
    jQuery('.waxon_progress').each(function () { var pWrap = jQuery(this); pWrap.waypoint({ handler: function () { tdProgress(pWrap); }, offset: '90%' }); }); function waxon_tm_jarallax() {
    ...
    waxon_tm_jarallax(); function edrea_tm_hamburger() {
        var hamburger = jQuery('.hamburger'); var mobileMenu = jQuery('.waxon_tm_mobile_menu .dropdown'); hamburger.on('click', function () {
    ...
    waxon_tm_nav_bg(); function waxon_tm_cursor() { var myCursor = jQuery('.mouse-cursor'); if (myCursor.length) { if ($("body")) { const e = document.querySelector(".cursor-inner"), t = document.querySelector(".cursor-outer"); let n, i = 0, o = !1; window.onmousemove = function (s) { o || (t.style.transform = "translate(" + s.clientX + "px, " + s.clientY + "px)"), e.style.transform = "translate(" + s.clientX + "px, " + s.clientY + "px)", n = s.clientY, i = s.clientX }, $("body").on("mouseenter", "a, .cursor-pointer", function () { e.classList.add("cursor-hover"), t.classList.add("cursor-hover") }), $("body").on("mouseleave", "a, .cursor-pointer", function () { $(this).is("a") && $(this).closest(".cursor-pointer").length || (e.classList.remove("cursor-hover"), t.classList.remove("cursor-hover")) }), e.style.visibility = "visible", t.style.visibility = "visible" } } }; waxon_tm_cursor(); function waxon_tm_partners() {
        var carousel1 = jQuery('.waxon_tm_service .owl-carousel'); var rtlMode = false; if (jQuery('body').hasClass('rtl')) { rtlMode = 'true'; }
    ...
    waxon_tm_partners(); function waxon_tm_imgtosvg() {
    ...
    waxon_tm_portfolio(); function waxon_tm_myload() { var speed = 1000; setTimeout(function () { jQuery('.waxon_tm_preloader').addClass('loaded'); }, speed); setTimeout(function () { jQuery('.waxon_tm_hero .background .myOverlay').addClass('loaded'); }, speed + 1000); setTimeout(function () { jQuery('.waxon_tm_topbar').addClass('opened'); }, speed + 2000); setTimeout(function () { waxon_tm_isotope(); }, speed + 4000); }
